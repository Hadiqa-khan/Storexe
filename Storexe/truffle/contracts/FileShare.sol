...
contract FileShare is ERC721, ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => tokenId) private FileRecord ; 
    mapping(address => files[]) public FileLog; 

    function safeMint(address to, string memory uri) public onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        FileLog[msg.sender].push(new files(tokenId,msg.sender,to,uri));
    ...
    function share_files() public view returns(files[] memory)
   {
     ...
     {
        if(FileLog[i].share_add==msg.sender)
        {
            count=count+1;
        }
     ...
     for(uint256 i=0;i<_tokenIdCounter.current();i++)
     {
        if(FileLog[i].share_add==msg.sender)
        {
                files storage share_list =FileLog[i];
                obj[x]=share_list;
                x=x+1;
        }
     }
